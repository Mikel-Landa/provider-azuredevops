...
import (
	...
	"github.com/Mikel-Landa/provider-azuredevops/apis/v1beta1"
)
...
const (
	...
	secretKeyOrgServiceURL       = "orgServiceURL"
	secretKeyPersonalAccessToken = "personalAccessToken"
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := creds[secretKeyOrgServiceURL]; ok {
			ps.Configuration[keyOrgServiceURL] = v
		}
		if v, ok := creds[secretKeyPersonalAccessToken]; ok {
